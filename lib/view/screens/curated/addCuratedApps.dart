...
class AddCuratedApps extends StatefulWidget {
   AddCuratedApps({Key? key, required this.appsList, required this.appsIconsList, required this.shelfName, required this.linkUrl, required this.selectedColor}) : super(key: key);
...
class _AddCuratedAppsState extends State<AddCuratedApps> with SingleTickerProviderStateMixin{
              ...
              floatingActionButton: GestureDetector(
                onTap: () {
                  Navigator.of(context).pushReplacement(
                    MaterialPageRoute(builder: (context) => CuratedAppsView(appsList: widget.appsList, appsIconsList: widget.appsIconsList, dialog: true, shelfName: widget.shelfName, linkUrl: widget.linkUrl, selectedColor: widget.selectedColor,)));
                },
                child: Container(
                        ...
                        child: TextFormField(
                          onChanged: (value) {
                            if(value.length != 0){
                                ...
                                var queryy = FirebaseFirestore.instance.collection('apps').where('title', isGreaterThanOrEqualTo: value).snapshots();
                                query = queryy;
                            } else{
                                var queryy = FirebaseFirestore.instance.collection('apps').snapshots();
                                query = queryy;
                            }
                            setState(() {
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          ...
                          onChanged: (String? newValue) { 
                              ...
                              if(newValue == 'Latest'){
                                var queryy = FirebaseFirestore.instance.collection('apps').orderBy('uploadedAt', descending: true).snapshots();
                                query = queryy;
                              } else if (newValue == 'Older'){
                                var queryy = FirebaseFirestore.instance.collection('apps').orderBy('uploadedAt').snapshots();
                                query = queryy;
                              } else if (newValue == 'Popularity'){
                                var queryy = FirebaseFirestore.instance.collection('apps').orderBy('rating', descending: true).snapshots();
                                query = queryy;
                              } else{
                                var queryy = FirebaseFirestore.instance.collection('apps').snapshots();
                                query = queryy;
                      ...
                      return ListView(
                        ...
                        children:
                            snapshot.data!.docs.map((document) {
                              ...
                              if(widget.appsList.contains(document.id.toString()))
                                _selected = true;
                              ...
                              return GestureDetector(
                                onTap: () {
                                  if(widget.appsList.contains(document.id)){
                                    ...
                                    widget.appsIconsList.add(document['icon'].toString());
                                  }
                                  setState(() {
...
class AppDisplayBox extends StatelessWidget {
  const AppDisplayBox({
    ...
    this.category,
    this.appId,
  this.ratting,this.developer,this.description,this.iconName,this.link,this.backgroundColor, this.color, this.selectedIcon
  }) : super(key: key);
          ...
          title: Row(
            ...
            children: [
              ...
              selectedIcon! ?  SizedBox(
                width: 20,
                height: 20,
                child: Image.asset('assets/icons/selectedIcon.png')) : SizedBox(),
            ],
          ),
        ),
      ),  

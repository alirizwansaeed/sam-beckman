...
addReview({var appId, var ratting, var description,var context})async{
  ...
  FirebaseFirestore.instance.collection('apps').doc(appId).update({'reviews': FieldValue.arrayUnion(review)}).then((value) => Navigator.of(context).pop());
  FirebaseFirestore.instance.collection('users').doc(FirebaseAuth.instance.currentUser!.uid).update({'reviews': FieldValue.increment(1)}).then((value) => Navigator.of(context).pop());
